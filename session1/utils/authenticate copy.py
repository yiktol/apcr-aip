...
try:
    ...
    print("Successfully retrieved Cognito credentials:")
    for key, value in credentials.items():
        if value:
            # Show first few characters of values for security
            masked_value = value[:4] + "..." if value else "None"
            print(f"{key}: {masked_value}")
        else:
            print(f"{key}: Not found in secret")
except Exception as e:
    print(f"Failed to retrieve credentials: {str(e)}")

COGNITO_DOMAIN = credentials.get("COGNITO_DOMAIN")
CLIENT_ID = credentials.get("COGNITO_APP_CLIENT_ID")
CLIENT_SECRET = credentials.get("COGNITO_APP_CLIENT_SECRET")
APP_URI = credentials.get("COGNITO_REDIRECT_URI")
# ------------------------------------
...
def get_auth_code():
    ...
    Gets auth_code state variable.    Returns:
        auth_code: The authorization code from query parameters.
    """
