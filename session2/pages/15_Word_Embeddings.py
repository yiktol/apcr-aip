...
def render_results_tabs(df_embeddings):
    ...
    with tab1:
        render_2d_visualization(df_vectors)
    
    with tab2:
        render_3d_visualization(df_vectors)
            
    with tab3:
        render_similarity_analysis(df_embeddings)
        
    with tab4:
        render_raw_data(df_vectors)

    return df_vectors

def render_2d_visualization(df_vectors):
    ...
    col1.metric("Words Visualized", len(df_vectors))
...
def render_3d_visualization(df_vectors):
...
def render_similarity_analysis(df_embeddings):
...
def render_raw_data(df_vectors):
...
def render_word_comparison_section():
...
def generate_embeddings(words):
    ...
    if bedrock:
        ...
        for i, word in enumerate(words):
            ...
            # Update progress
            progress_percentage = (i + 1) / len(words)
    ...
    with left_column:
        ...
        with st.form("embedding_form"):
            ...
            if generate_button and all_selected_words:
                generate_embeddings(all_selected_words)
        ...
        if st.session_state.embeddings_generated and len(st.session_state.embedding_list) > 0:
            ...
            df_vectors = render_results_tabs(df_embeddings)

            # Word comparison section
            render_word_comparison_section()
